...
function createApp(database) {
  ...
  app.get("/prices", (req, res) => {
    ...
    const parsedDate = parseTemporalDate(req.query.date);
  ...
  function calculateCost(age, type, date, baseCost, temporalDate) {
    if (type === "night") {
  ...
  function calculateCostForDayTicket(age, date, baseCost, temporalDate) {
  ...
  function calculateReduction(date, temporalDate) {
    let reduction = 0;
  ...
  function isHoliday(date, temporalDate) {
    ...
    const day = date;
    const temporalDay = temporalDate;
    for (let row of holidays) {
      let parsedHoliday = Temporal.PlainDate.from(row.holiday);
      if (
        (day || temporalDate) &&
        (day.getFullYear() || temporalDate.year ) === parsedHoliday.year &&
        day.getMonth() === (parsedHoliday.month - 1) &&
        day.getDate() === parsedHoliday.day
      ) {
        return true;
      }
    }
